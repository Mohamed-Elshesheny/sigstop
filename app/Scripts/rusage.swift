...
guard status == 0 else {
    FileHandle.standardError.write(Data("no process \(pid) to measure\n".utf8))
    exit(1)
}
var timebase = mach_timebase_info_data_t()
mach_timebase_info(&timebase)
let cpu = Double(info.ri_user_time + info.ri_system_time) * Double(timebase.numer) / Double(timebase.denom)
print(String(format: "%.0f %llu %llu %llu", cpu, info.ri_interrupt_wkups, info.ri_energy_nj, info.ri_phys_footprint))
