...
function VerdictChip({ verdict }: { verdict: Verdict }) {
  const label = copy.ladder.verdicts[verdict];
  return (
    <span
      className={cn(
        "shrink-0 rounded px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.16em]",
        verdict === "block"
          ? "bg-suspend text-accent-fg font-semibold"
          : verdict === "hold"
...
function Phase({
  ...
  phase: (typeof copy.mute.phases)[number];
  index: number;
}) {
  ...
  return (
    <li
      className={cn(
        ...
        quiet ? "border-line-hi border-dashed" : held ? "border-suspend/50 border-dashed" : "border-suspend",
      )}
    >
      <span
        aria-hidden
  ...
  return (
    <Section id="meetings">
      <div ref={ref} className="reveal" data-visible={visible}>
        ...
        <h3 className="mt-14 font-mono text-base font-bold tracking-tight">{copy.ladder.title}</h3>
          ...
          {copy.ladder.items.map((item) => (
            <li key={item.state} className="bg-bg p-5">
              <div
                className={cn(
                  "border-l-2 pl-4",
                  RAIL[item.verdict as Verdict],
                  item.verdict === "defer" ? "border-line-hi" : "border-suspend",
                )}
              >
        ...
        <div className="mt-14">
          ...
          {/* The ceilings. Numbers first, because the objection to any hold is
              "and how long does it hold for", and the answer is short. */}
          <h4 className="mt-10 font-mono text-[12px] font-semibold uppercase tracking-[0.14em] text-fg-faint">
            {copy.bounds.title}
          </h4>
        ...
        <div className="mt-14 rounded-xl border border-line bg-surface/50 p-6 sm:p-8">
          ...
          <ul className="mt-3 max-w-3xl space-y-2.5">
            {copy.gap.also.map((item) => (
              <li key={item} className="flex gap-3 text-pretty text-sm leading-relaxed text-fg-muted">
                <span aria-hidden className="select-none pt-px font-mono text-fg-faint">
                  {"∅"}
                </span>
                {item}
              </li>
          ...
          <p className="mt-7 max-w-3xl border-l-2 border-suspend pl-4 text-pretty leading-relaxed text-fg">
            <span className="font-mono text-[12px] font-semibold uppercase tracking-[0.14em] text-fg-faint">
