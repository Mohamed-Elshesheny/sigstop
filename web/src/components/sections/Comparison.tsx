  ...
  return (
    <Section>
      <div ref={ref} className="reveal" data-visible={visible}>
        ...
        {/* The two rows we lose, explained rather than buried. */}
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {copy.footnotes.map((f, i) => (
            <p
              key={i}
              className="rounded-lg border border-line bg-surface/30 px-4 py-3 text-[13px] leading-relaxed text-fg-muted"
            >
              {f}
            </p>
          ))}
